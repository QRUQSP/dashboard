...
/*    list($package, $module, $panel) = explode('.', $args['panel_ref']);
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.dashboard.43', 'msg'=>'Unable to load panel', 'err'=>$rc['err']));
    }
    $fn = $rc['function_call'];
    $rc = $fn($ciniki, $args['tnid'], array());
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.dashboard.47', 'msg'=>'Unable to load panel', 'err'=>$rc['err']));
    }
